...
-------------------------------------------------------------------------------
-- name: mobf_read_world_specific_data(identifier)
--
...
--! @return table containing data
-------------------------------------------------------------------------------
function mobf_read_world_specific_data(identifier)
	...
	if file ~= nil then
		...
		if data_raw ~= nil then
			retval = minetest.deserialize(data_raw)
		end
	end
...
-------------------------------------------------------------------------------
-- name: mobf_write_world_specific_data(identifier, data)
--
--! @brief read world specific data and provide as lua table
--
--! @param identifier some string to uniquely identify this dataset
...
-------------------------------------------------------------------------------
function mobf_write_world_specific_data(identifier, data)

	local world_path = minetest.get_worldpath()
	local file,error = io.open(world_path .. "/" .. identifier,"w")

	if error ~= nil then
		minetest.log(LOGLEVEL_ERROR,"MOBF: failed to open world specific data file for " .. identifier)
	end
